...
Matrix::Matrix(int nrLines, int nrColumns) {
    // this function is the constructor of the matrix 
    // theta(1)
    nrCols = nrColumns;
    nrRows = nrLines;
...
void Matrix::resize(int newCapacity) {
    // BC: theta(1) (if size==0 or ==1) WC,AC: theta(n)
...
void Matrix::append(TELEM e) {
    // BC theta(1) (size<max_capacity), WC theta(n), AC theta(1) (1(size=0)+1+1+...+1(size=max_cap-1)+n)/n= n+n /n =2 =ct=theta(1)
...
TElem Matrix::modify(int i, int j, TElem e) {
    ...
    // BC theta(1)("invalid" input), WC theta(n), AC theta(n)
    if (!(0 <= i && i < nrRows && 0 <= j && j < nrCols))
        throw exception();
    TELEM x;
    x = make_tuple(i, j, e);
